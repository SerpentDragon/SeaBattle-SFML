...
DropDownList::DropDownList(std::shared_ptr<RenderWindow> window)
    : window_(window), 
    list_(Vector2f(DR_LS::dropDownListSize, DR_LS::dropDownListSize)),
    x_(DR_LS::droDownListXPos), 
    y_(DR_LS::dropDownListYPos),
    width_(DR_LS::dropDownListSize), height_(DR_LS::dropDownListSize)
{
    list_.setFillColor(DR_LS::BROWN);
...
void DropDownList::drawList()
{
    ...
    height_ = onList(x, y) ? DR_LS::dropDownListSize * items_.size() : DR_LS::dropDownListSize;
    ...
    if (height_ > DR_LS::dropDownListSize)
...
{
    const int itemSize = 
        DR_LS::dropDownListSize - Item::itemOutlineThickness * 2;
    ...
    items_ = {
        ...
        { 0, 0, itemSize, 
            std::make_shared<Texture>(*TextureManager::getManager()->
                getTexture("textures/languages/en_US")),
            "en_US" },
    ...
    for(size_t i = 0; i < items_.size(); i++)
    {
        items_[i].itemRect_.setPosition(x_ + Item::itemOutlineThickness,
            y_ + DR_LS::dropDownListSize * i + Item::itemOutlineThickness);
    }
}
...
{
    Color outlineColor = highlight ? DR_LS::DARK_BROWN : DR_LS::BROWN;
